...
namespace FreyaManager.Features.Employees.Services;
...
public class CheckEmployeeService
{
    ...
    //Copy from Freya/Employees/Services/CheckEmployeeService
    // Unified on future
    ...
    public async Task<Result<NewEmployeeState>> CheckEmployeeStateForACompany(string email, Guid companyId)
    {
        Result<NewEmployeeState> result = Error();
        var response = await taskHelperFactory.
                               CreateInternetAccessViewModelInstance(logger).
                               WithErrorHandling(OnCheckEmployeeStateError).
                               TryExecuteAsync(
                               () => employeeAPIService.CheckEmployeeStateForRegisterForACompany(new CheckEmployeeStateForRegisterForCompanyResquest()
                               {
                                   EmployeeEmail = email,
                                   ConpanyId = companyId,
                               }));
